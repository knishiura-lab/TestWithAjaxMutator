...
public class MediaWikiBookCreatorTest extends TestBase {
	...
	public void test40() {
		...
		action.moveToElement(targetPage);
		action.perform();
		By popupLocator = By.xpath("//*[@id=\"collectionpopup\"]/a");
		try {
			Thread.sleep(3000);
	...
	public void test39() {
		...
		String linkText = findElement(popupLocator).getText();
		assertTrue(!linkText.equals(""));
	}
	...
	public void test38() {
		...
		WebElement popup = findElement(popupLocator);
		String title = findElement(xpath).getAttribute("title");
		assertFalse(popup.isDisplayed() && !title.isEmpty());
	}
	...
	public void test31() {
		...
		action.moveToElement(targetPage);
		action.perform();
		By popupLocator = By.xpath("//*[@id=\"collectionpopup\"]/a/img");
		try {
			Thread.sleep(3000);
		} catch (InterruptedException e1) {
			e1.printStackTrace();
		...
		try {
			WebElement popupImg = findElement(popupLocator);
	...
	public void testAddRemoveThisPage() {
		...
		By xpath = By
				.xpath("//*[@id=\"coll-remove_article\" or @id=\"coll-add_article\"]");
		WebElement targetPage = findElement(xpath);
		...
		xpath = By
				.xpath("//*[@id=\"coll-add_article\" or @id=\"coll-remove_article\"]");
	...
	private void testOnCase(WebElement targetPage, Object... expects) {
		...
		if (!popup.isDisplayed())
			return;
		assertEquals(
				isNear(popup.getLocation(), new Point(
						targetPage.getLocation().x + targetPage.getSize().width
								/ 2 + 2, targetPage.getLocation().y
								+ targetPage.getSize().height / 2 + 2), 5),
				true);
		int fuzz = 5, e = 10;
		...
		int[] x_positions = { left - fuzz - e, left - fuzz + e,
				left + width + fuzz + e };
		int[] y_positions = { top - fuzz - e, top - fuzz + e,
				top + height + fuzz + e };
		for (int x : x_positions) {
			for (int y : y_positions) {
				...
				action.moveByOffset(x - (left + width / 2), y
						- (top + height / 2));
				action.perform();
				try {
					Thread.sleep(1000);
	...
	private void before() {
		...
		handleLogin();
		handleStart();
		waitUntil(ExpectedConditions.presenceOfAllElementsLocatedBy(By
				.className("collection-creatorbox")));
	}
	...
	private void handleStart() {
		...
		findElement(By.id("coll-create_a_book")).findElement(By.tagName("a"))
				.click();
		waitUntil(ExpectedConditions.presenceOfElementLocated(By
				.className("collection-button")));
		List<WebElement> collectionButtons = findElements(By
				.className("collection-button"));
	...
	private void handleLogin() {
		...
		waitUntil(ExpectedConditions.presenceOfElementLocated(By.id("wpName1")))
				.sendKeys("testuser");
